...
fname = INDIR_LS + 'Rapid decrease/Beginning_of_rapid_decrease1_new.txt'
begin_RG_LS = doy(fname) 
begin_RG_LS = np.array(begin_RG_LS, dtype=float)
begin_RG_LS[0]=np.nan
begin_RG_LS[3]=np.nan

slope1_LS, intercept1_LS, r_value1_LS, p_value1_LS, std_err1_LS = stats.linregress(np.delete(years_LS,[0,3]), np.delete(begin_RG_LS,[0,3])) 
print slope1_LS, r_value1_LS, p_value1_LS, std_err1_LS   
line1_LS = slope1_LS*years_LS+intercept1_LS

fname_rgrth= INDIR_LS + 'End of FI season/End_season_new.txt'
end_rgrth = doy(fname_rgrth) 
end_rgrth = np.array(end_rgrth, dtype=float)
end_rgrth[0]=np.nan
#end_rgrth = np.delete(end_rgrth,[3,14])
slope2_LS, intercept2_LS, r_value2_LS, p_value2_LS, std_err2_LS = stats.linregress(np.delete(years_LS,0), np.delete(end_rgrth,0)) 
